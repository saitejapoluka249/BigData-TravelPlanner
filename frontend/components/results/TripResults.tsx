...
const LoadingState = () => {
  ...
  return (
    <div id="loading-state-container" className="w-full h-[calc(100vh-140px)] flex flex-col items-center justify-center p-8 bg-theme-bg rounded-2xl border border-theme-surface shadow-sm animate-in fade-in duration-300">
      <div id="loading-icon-container" className="relative flex items-center justify-center w-24 h-24 mb-6">
        ...
        <div id="loading-active-icon" className="z-10 text-4xl drop-shadow-sm transition-transform scale-110">
          {icons[iconIndex]}
        </div>
      </div>
      <h3 id="loading-heading" className="text-xl font-black text-theme-text mt-4 animate-pulse">
        Crafting your itinerary...
      </h3>
      <p id="loading-subtext" className="text-theme-text/70 text-sm mt-2 text-center max-w-sm">
...
export default function TripResults({
  ...
  return (
    <div id="trip-results-main-container" className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div id="trip-results-tabs-wrapper" className="sticky top-0 z-20 bg-theme-bg backdrop-blur-sm pt-1 mb-4 border-b border-theme-surface">
        <div id="trip-results-tabs-container" className="flex w-full">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div id={`tab-active-indicator-${tab.id}`} className="absolute bottom-[-1px] left-0 right-0 h-[3px] bg-theme-primary rounded-t-full" />
                )}
              </button>
            );
          })}
      ...
      <div id="trip-results-content-container" className="w-full pb-8">
        {activeTab === "flights" && (
          <FlightCard flights={data?.flightData || []} />
        )}
        {activeTab === "drive" && (
          <div id="drive-tab-content" className="flex flex-col gap-4">
            {showFlights && !hasFlights && (
              <div id="driving-fallback-info" className="p-4 bg-theme-muted/20 text-theme-primary rounded-xl border border-theme-muted flex items-center gap-3">
                <span id="driving-fallback-icon" className="text-xl">ℹ️</span>
                <p id="driving-fallback-text" className="text-sm font-medium">
